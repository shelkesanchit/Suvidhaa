...
/**
 * ApplicationReceipt — Receipt dialog after successful form submission.
 *
 * Props:
 *  open             {boolean}
 *  onClose          {function}
...
const ApplicationReceipt = ({
        ...
        <head>
          ...
          <style>
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .header { background: #1565c0; color: #fff; padding: 20px 24px; border-radius: 8px 8px 0 0; }
            .header h1 { font-size: 22px; margin-bottom: 2px; }
            .header p { font-size: 12px; opacity: 0.85; }
            .app-number-box { background: #e8f0fe; border: 2px solid #1565c0; border-radius: 8px; padding: 16px; text-align: center; margin: 16px 0; }
            .app-number-box .label { font-size: 11px; color: #666; text-transform: uppercase; letter-spacing: 1px; }
            .app-number-box .number { font-size: 32px; font-weight: bold; color: #1565c0; letter-spacing: 4px; }
            .section { margin-bottom: 16px; }
            .section-title { font-size: 13px; font-weight: bold; color: #1565c0; border-bottom: 2px solid #e8eaf6; padding-bottom: 6px; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 0.5px; }
  ...
  const handleEmailReceipt = async () => {
    ...
    try {
      await api.post('/electricity/otp/send-receipt', {
        email,
      ...
      <DialogContent sx={{ p: 0 }}>
        ...
        <Box ref={receiptRef} sx={{ px: 3, pt: 3, pb: 1 }}>
          ...
          <Box className="header" sx={{ display: 'none' }}>
            <h1>SUVIDHA</h1>
            <p>Smart Utility Kiosk Services – Government of India</p>
          </Box>
          ...
          >
            ...
            <Typography variant="h4" sx={{ color: '#1565c0', fontWeight: 800, letterSpacing: 3, mt: 0.5 }}>
              {applicationNumber}
            </Typography>
          </Paper>
          ...
          {/* Summary */}
          <Box sx={{ mb: 2.5 }}>
            <Typography variant="subtitle2" sx={{ color: 'primary.main', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, mb: 1, pb: 0.5, borderBottom: '2px solid', borderColor: 'grey.200' }}>
              Application Summary
            </Typography>
            <Table size="small">
              <TableBody>
          ...
          {/* Next Steps */}
          <Box sx={{ background: '#f8f9ff', borderRadius: 2, p: 2, mb: 2 }}>
